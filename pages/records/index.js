...
Page({
  ...
  onShow() {
    ...
    if (!this.data.editing) this.loadOptions().then(() => this.load())
  },
  onPullDownRefresh() { this.load().finally(() => wx.stopPullDownRefresh()) },
  async loadOptions() {
    try {
      const overview = await api.families.overview(store.getFamilyId())
      this.setData({ elderOptions: [{ elder_id: '', name: '全部长辈' }, ...(overview.elders || [])] })
  ...
  applyFilter() {
    ...
    this.setData({ filteredList: keyword ? this.data.list.filter((item) => String(item.drug_name).toLowerCase().includes(keyword)) : this.data.list })
  },
  add() {
    if (!store.canEdit()) { toast('当前角色仅可查看'); return }
    wx.setStorageSync('elder_medication.medication_elder_id', '')
  ...
  async save() {
    ...
    if (!f.dose || !f.start_date) { toast('请完整填写剂量和日期'); return }
    this.setData({ saving: true })
    try {
      await api.records.update(f.record_id, {
        dose: f.dose, frequency: f.frequency, start_date: f.start_date, end_date: f.end_date || null,
      })
      toast('修改成功', 'success')
  ...
  async remove(event) {
    ...
    if (!(await confirm('相关提醒规则将一并删除，确定继续？', '删除用药记录'))) return
    try { await api.records.remove(id); toast('已删除', 'success'); await this.load() } catch (error) { showError(error) }
  },
})
