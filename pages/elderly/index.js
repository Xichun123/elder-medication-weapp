...
Page({
  ...
  onShow() {
    if (this.data.elder) this.loadReminders()
  },
  ...
  setDateText() {
    ...
    const h = d.getHours()
    this.setData({
      today: `${d.getMonth() + 1}月${d.getDate()}日 ${week}`,
      greeting: h < 6 ? '凌晨好' : h < 11 ? '早上好' : h < 14 ? '中午好' : h < 18 ? '下午好' : '晚上好',
    })
  },

  async loadElders() {
    try {
      const elders = unwrap(await api.elders.list())
  ...
  onElderChange(event) {
    this.setData({ elderIndex: Number(event.detail.value) }, () => this.loadReminders())
  },
  ...
  playCurrent() {
    if (this.data.currentReminder) this.playText(this.data.currentReminder.voice_text)
  },

  openAi() {
    const elder = this.data.elder
    if (!elder) return
    wx.navigateTo({ url: `/pages/ai-chat/index?mode=elder&elder=${elder.elder_id}` })
  },
  ...
  async takeCurrent() {
    if (!this.data.currentReminder) return
    await this.takeById(this.data.currentReminder.rule_id)
  },
  ...
  startTimer() {
    ...
    this.timer = setInterval(async () => {
      ...
      const current = this.data.currentReminder
      if (current && this.lastPlayedId !== current.rule_id) {
        this.lastPlayedId = current.rule_id
        this.playText(current.voice_text)
      }
    }, 60000)
  },
})
