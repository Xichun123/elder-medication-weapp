...
Page({
  ...
  async loadReminders() { const selected = this.data.elders[this.data.elderIndex]; if (!selected) return; try { const [elder, data] = await Promise.all([api.elders.get(selected.elder_id), api.reminders.list({ elder: selected.elder_id })]); const reminders = unwrap(data); const currentReminder = reminders.find((item) => item.status === 'pending') || null; this.setData({ elder, reminders, currentReminder, nextReminder: currentReminder }) } catch (error) { showError(error) } },
