...
Page({
  ...
  openEdit(event) {
    ...
    if (row) this.setData({ editing: true, isEdit: true, form: { ...row }, formCategoryIndex: formCategoryIndexOf(row.category) })
  },
  ...
  async save() {
    ...
    if (!f.drug_id || !f.generic_name || !f.category) { toast('请完整填写必填项'); return }
    this.setData({ saving: true })
    try {
      if (this.data.isEdit) await api.drugs.update(f.drug_id, f)
      else await api.drugs.create(f)
  ...
  async remove(event) {
    ...
    if (!(await confirm('若已有用药记录引用，本地数据引擎会拒绝删除。确定继续？', '删除药物'))) return
