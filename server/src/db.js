...
/** 系统药库按通用名增量补齐，升级旧数据库时无需清库。 */
function seedSystemCatalog(database) {
  const ts = nowIso()
  const findDrug = database.prepare('SELECT id FROM drugs WHERE home_id IS NULL AND generic_name = ? LIMIT 1')
  ...
  commonDrugs.forEach((drug) => {
    if (findDrug.get(drug.generic_name)) return
    insertDrug.run(
      drug.drug_id,
      drug.generic_name,
      drug.trade_name,
  ...
  const contras = [
    ['Csys01', '阿司匹林', null, '布洛芬', 'co_administration', 'severe', '增加消化道出血风险'],
    ['Csys02', '硝苯地平', null, '西柚', 'diet', 'middle', '西柚升高血药浓度致低血压'],
    ['Csys03', '二甲双胍', null, '酒精', 'diet', 'severe', '乳酸酸中毒风险'],
    ['Csys04', '头孢克肟', null, '酒精', 'diet', 'severe', '双硫仑样反应'],
    ['Csys05', '阿莫西林', null, '活菌制剂', 'co_administration', 'light', '抗菌药灭活益生菌'],
  ]
  const insertContra = database.prepare(`
    INSERT OR IGNORE INTO contraindications (
      id, home_id, drug_a_id, drug_b_id, drug_b_text, contra_type, severity, note, created_at, updated_at
    ) VALUES (?, NULL, ?, ?, ?, ?, ?, ?, ?, ?)
  `)
  contras.forEach(([id, drugName, drugBId, drugBText, type, severity, note]) => {
    const drug = findDrug.get(drugName)
    if (drug) insertContra.run(id, drug.id, drugBId, drugBText, type, severity, note, ts, ts)
  })
}
